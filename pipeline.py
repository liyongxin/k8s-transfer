...
def trans_pipeline(pipeline):
    ...
    for task in tasks:
        ...
        if task["type"] == "exec":
            ...
            new_task["data"] = transed_data
            trans_tasks.append(new_task)
        if task["type"] == "manual-control" and task["data"]["exec_enabled"] == "true":
            transed_data = trans_manual_control_task(task_data)
            new_task["data"] = transed_data
            trans_tasks.append(new_task)
        if task["type"] == "test-container":
    ...
    pipeline["stages"]["task"][0] = trans_tasks
    return pipeline
...
def trans_test_container_task(task_data):
    ...
    transed_data["k8s_namespace_uuid"] = ""
    transed_data["k8s_namespace"] = k8s_namespace_resource["uuid"]
    transed_data["env"] = get_svc_env({
...
def trans_exec_task(task_data):
    ...
    if "instance" in transed_data:
        ...
        transed_data["containers"] = [service_name + "-0"],
    return transed_data
